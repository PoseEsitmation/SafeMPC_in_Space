...
class GTCost():
    ...
    def _space_task(self, task_id):
        from hypercrl.envs.space_tasks import get_task_spec
        spec = get_task_spec(self.env_name, task_id or 0)
        return spec.beta, spec.alpha, torch.tensor(spec.B(), dtype=torch.float32, device=self.device)
    ...
    def __call__(self, x, u, t, task_id):
        ...
        if self.env_name.startswith("half_cheetah"):
            ...
            beta, alpha, B = self._space_task(task_id)
            ...
            # torque penalty (tau = B u, torque_max=2*sqrt(3))
            torque_norm = torch.norm(u @ B.T, p=2, dim=-1) / (2.0 * np.sqrt(3))
            ...
            penalty_koz = torch.where(
                theta_margin <= 0,
                torch.full_like(theta_margin, beta),
                beta * torch.exp(-alpha * theta_margin),
            )
