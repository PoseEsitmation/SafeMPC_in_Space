...
class GTCost():
    ...
    def __call__(self, x, u, t, task_id):
        ...
        if self.env_name.startswith("lqr"):
            ...
            alive = (x[:, 0] > 1.0) * (x[:, 0] < 2.0)
            ctrl_cost = 0.1 * (u**2).sum(dim=-1)
            reward = alive.float() * (5.0 + 1.25 * x[:, 22] - ctrl_cost)
            cost = -reward
            ...
            for i, corner in enumerate([c1, c2, c3, c4]):
                diff = torch.norm(
                    corner - self.suite_push_goal[i], p=2, dim=-1)
            ...
            for i, corner in enumerate([c1, c2, c3, c4]):
                diff = torch.norm(
                    corner - self.suite_slide_goal[i], p=2, dim=-1)
                reward += 1 - torch.tanh(10.0 * diff)
            reward -= 0.1 * torch.norm(u, p=2, dim=-1)
            cost = -reward
            ...
            reward_dist = -dist
            reward_log_dist = -torch.log(torch.pow(dist, 2) + 5e-3) - 5.0
            euler_diff = quat2euler(quat_mul(
                np.tile(self.des_q_w, (x.size(0), 1)), x[:, 3:7].detach().cpu().numpy()))
            euler_diff = torch.tensor(
                euler_diff, dtype=x.dtype, device=x.device)
    ...
    def reward(self, x, u, t, task_id):
        ...
        def pick_place_v1():
            ...
            def reachReward():
                ...
                reachDistxy = torch.norm(
                    objPos[:, :-1] - fingerCOM[:, :-1], p=2, dim=-1)
...
def test_metaworld():
    ...
    for task_id in range(2, 3):
        ...
        for i in range(150):
            ...
            x_tt, rew, done, truncated, info = env.step(u)
