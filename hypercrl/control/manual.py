...
class Manual(object):
    def __init__(self, env, x_dim, a_dim, horizon, dynamics, device):
        self.x_dim = x_dim
        self.a_dim = a_dim
        self.env = env
    ...
    def plan(self, x, task_id):
        ...
        if self.env == "pusher":
            goal = 0.06
            if x[2] > goal:
                action = torch.tensor([0., 0.], device=self.device)
            else:
                action = torch.randn(2, device=self.device) * torch.tensor([0.3, 0.3], device=self.device) \
